...
/*function getAdminOptions(id){
	//获取user
	 $.ajax({     
	        type: "GET",     
	        url: "ajax_admin_data?community_id="+ id + "&random=" + Math.random(),     
	        dataType:   "json",     
	        success: function(json){  
	        	$("#user_id").empty();
	        	item_str = '<option value="0">请选择</option>';
...
function audit_submit(){
	...
	if(user_id == 0){
	 ...
	 $.ajax({     
	        ...
	        success: function(json){  
	        	if(json.flag == 'ok'){
	        		$("#center-column").load("../../static/web/admin_templates/common_admin.html?random=" + Math.random());
	        	}
	        }      
	 })
}
