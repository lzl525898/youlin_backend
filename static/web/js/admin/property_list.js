...
function show_audit_data(list) {
	$(".listing > tbody").empty();
	//title部分
	var item_str = '<tr>';
	item_str += '<th class="first" width="20%">申请人</th>';
	item_str += '<th  width="25%">城市</th>';
	item_str += '<th  width="30%">小区</th>';
...
function do_page() {
	$.getJSON(
		"ajax_audit_list?type=" + get_menu_param("type") + "&page=" + curr_page + "&random=" + Math.random(),
		function(data) {
			show_audit_data(data.data.list);
...
$(function() {
	var url = null;
	//alert(get_menu_param("page"));
	if(get_menu_param("page")) {
		curr_page = get_menu_param("page");
		url = "ajax_audit_list?type=" + get_menu_param("type") + "&page=" + get_menu_param("page") + "&random=" + Math.random();
	}else{
		url = "ajax_audit_list?type=" + get_menu_param("type") + "&random=" + Math.random();
	}
	$.getJSON(
		url,
		function(data) {
	...
	$("#add_btn").click(function() {
		$("#menu_param").val("type:" + get_menu_param("type"));
		$("#center-column").load("../../static/web/admin_templates/auditing_add.html?random=" + Math.random());
